@extends('layouts.admin', [
    'title'       => 'NRI Investment Desk CMS',
    'breadcrumbs' => [
        'Website Management' => route('admin.website.index'),
        'NRI Investment Page' => route('admin.website.nri'),
    ]
])
...
@include('admin.website.partials.cms-header', [
    'pageTitle'   => 'NRI Investment Desk — CMS',
    'pageSubtitle'=> 'Edit NRI hero, benefits, process, and CTA sections',
...
<div class="cms-split-layout">
    <div class="cms-edit-panel">
                ...
                @foreach($sections as $key => $sec)
                    <li class="nav-item">
                        <button class="nav-link {{ $loop->first ? 'active' : '' }}"
                                data-bs-toggle="tab" data-bs-target="#pane-nri-{{ $loop->index }}">
        ...
        <div class="tab-content">
            @forelse($sections as $key => $sec)
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pane-nri-{{ $loop->index }}">
                    @include('admin.website.partials.section-form', [
                        'sec'         => $sec,
                        'updateRoute' => route('admin.website.nri.section.update', $sec),
                    ])
                </div>
            @empty
    ...
    @include('admin.website.partials.preview-panel', ['previewUrl' => url('/nri')])
</div>
