@extends('layouts.admin', [
    ...
    'breadcrumbs' => [
        ...
        'Contact Page'       => route('admin.website.contact'),
    ]
])

@php
    $page = $page ?? \App\Models\Page::where('slug', 'contact')->first();
...
@include('admin.website.partials.cms-header', [
    'pageTitle'   => 'Contact & Site Visit Page — CMS',
    'pageSubtitle'=> 'Edit contact hero and information sections',
    'backRoute'   => route('admin.website.index'),
    'previewUrl'  => url('/contact'),
    'openUrl'     => url('/contact'),
])
@if(session('success'))
...
<div class="cms-split-layout">
    <div class="cms-edit-panel">
                ...
                @foreach($sections as $key => $sec)
                    <li class="nav-item">
                        <button class="nav-link {{ $loop->first ? 'active' : '' }}"
                                data-bs-toggle="tab" data-bs-target="#pane-ct-{{ $loop->index }}">
        ...
        <div class="tab-content">
            @forelse($sections as $key => $sec)
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pane-ct-{{ $loop->index }}">
                    @include('admin.website.partials.section-form', [
                        'sec'         => $sec,
                        'updateRoute' => route('admin.website.contact.section.update', $sec),
                    ])
                </div>
            @empty
    ...
    @include('admin.website.partials.preview-panel', ['previewUrl' => url('/contact')])
</div>
